...
class coRNNCell(nn.Module):
    def __init__(self, n_inp, n_hid, dt, gamma, epsilon, kernel_size, ** kwargs):
        super(coRNNCell, self).__init__()
        self.dt = dt
        self.gamma = gamma
        self.epsilon = epsilon
        # self.i2h = PixelwiseConv2d(n_inp + n_hid + n_hid, n_hid, 3, 12, 10, stride=1, padding=1, dilation=1)
        self.i2h = nn.Conv2d(n_inp + n_hid + n_hid, n_hid, kernel_size, stride=1, padding=1, dilation=1,
...
class coRNN(nn.Module):
    def __init__(self, n_inp, n_hid, n_out, dt, gamma, epsilon, kernel_size, **kwargs):
        ...
        self.n_hid = n_hid
        self.cell = coRNNCell(n_inp, n_hid, dt, gamma, epsilon, kernel_size)
...
class coRNNCell2(nn.Module):
    def __init__(self, n_hid, dt, gamma, epsilon, kernel_size, **kwargs):
...
class coRNN2(nn.Module):
    def __init__(self, n_inp, n_hid, n_out, dt, gamma, epsilon, seq_len, n_roll, kernel_size, **kwargs):
        ...
        self.readin = nn.Conv2d(seq_len*n_inp, 2*n_hid, kernel_size, stride=1, padding=1, dilation=1,
                                padding_mode="replicate")

        self.cell = coRNNCell2(n_hid, dt, gamma, epsilon, kernel_size)
